    ...
    if blank:
        ...
        for i in range(length):
            if i == 0:
                ...
                ymin, ymax = max(
                    0, split_pos[i][1][0] - margin), split_pos[i][1][1] + margin
                x1, y1, x2, y2 = 0, ymin, pic_len, ymax
                sub = image.crop((x1, y1, x2, y2))
            elif i == length-1:
                pre_info = split_pos[i - 1]
                margin = min(split_pos[i][1][0] - pre_info[1][1], 2)
                ymin, ymax = split_pos[i][1][0] - \
                    margin, min(h, split_pos[i][1][1] + margin)
                x1, y1, x2, y2 = 0, ymin, pic_len, ymax
                sub = image.crop((x1, y1, x2, y2))
            else:
                next_info = split_pos[i + 1]
                pre_info = split_pos[i - 1]
                margin = min(split_pos[i][1][0] - pre_info[1]
                             [1], next_info[1][0] - split_pos[i][1][0], 2)
                ymin, ymax = split_pos[i][1][0] - \
                    margin, split_pos[i][1][1] + margin
                x1, y1, x2, y2 = 0, ymin, pic_len, ymax
                sub = image.crop((x1, y1, x2, y2))
